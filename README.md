...
<details>
  ...
  - **Python을 배우는 이유**
    - 쉽고 간결한 문법
    - 활발한 파이썬 커뮤니티
    - 광범위한 응용 분야: 웹 개발, 데이터 분석, 인공지능, 자동화 등
  - **Python 알고리즘**
    - 직관적인 문법
    - 강력한 표준 라이브러리
    - 빠른 프로토타이핑
...
<details>
  ...
  - **변수**
    ...
    - 변수 할당: 표현식을 통해 변수에 값을 저장 (오른쪽에서 왼쪽으로)
      1. **할당 연산자(=)** 오른쪽의 표현식을 평가 → 값(메모리 주소) 생성
      2. 생성된 메모리 주소를 왼쪽 변수에 저장:
         - 존재하지 않는 변수 → 새 변수 생성
         - 기존 변수 → 메모리 주소 변경
  - **메모리와 객체**
...
<details>
  ...
  - **데이터 타입의 필요성**
    - 값의 구분 및 처리 방법 이해
    - 특정 데이터 타입에 적합한 연산 적용
    - 명시적 타입 지정으로 오류 예방 및 가독성 향상
  - **정수 (int)**
    - 정수를 표현하는 자료형 (e.g., `a=10`, `b=0`, `c=-5`)
    - 진수 표기법:
      - 2진수: `0b10` → 2
      - 8진수: `0o30` → 24
      - 16진수: `0x10` → 16
  - **실수 (float)**
    - 실수를 표현하며 근삿값으로 저장
    - **부동소수점 오차**: 2진수 변환 과정에서 발생하는 근사치
    - 해결책: `decimal` 모듈 등 활용
  - **Sequence Types**
    - 여러 값을 순서대로 나열 (`str`, `list`, `tuple`, `range`)
    - 주요 특징:
      - **순서(Sequence)**: 값들이 순서대로 저장
      - **인덱싱(Indexing)**: 특정 위치의 값 선택/수정
      - **슬라이싱(Slicing)**: 값의 부분 추출
      - **길이(Length)**: `len()` 함수로 값 개수 확인
      - **반복(Iteration)**: 반복문 활용 가능
    - **문자열 (str)**
      - 문자의 순서가 있는 변경 불가능한 시퀀스
      - 작은/큰 따옴표로 표현 (`'example'`, `"example"`)
      - 특수 문자: `\n`, `\t`, `\\`, `\'`, `\"`
